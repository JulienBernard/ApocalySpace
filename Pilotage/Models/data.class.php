...
class Data {
	...
	public function __construct( $viewBuildings, $viewTechnologies ) {
		...
		if( $this->_planet->getPopulation() > (int)$this->_managePopulationMax )
		{
			...
			$this->_natalityProduction = round(($capitalLevel * 8) + ($this->getTechnologyLevel($medicalResearchId, $this->getId()) * (0.05*12*$capitalLevel)) * $this->_globalSpeedMult - $overcrowding) * $this->_birthrateMultiplier;				/* Taux arbitraire (10 par niveau) */
